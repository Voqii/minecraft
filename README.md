...
Mirroring EssentialsX's old behaviour, going AFK is **silent** by default while
returning is announced (`* <name> is no longer AFK.`), so non-paid players are still
announced on return once EssentialsX's own AFK broadcasts are turned off. Resolution
order is: the player's own message (`/cm set afk|return`, typically a paid perk) → the
server-wide `custom_afk_message` / `custom_return_message` in `config.yml` if set → the
built-in default. Set `custom_afk_message` if you do want going-AFK announced for
everyone. Players can still mute themselves entirely with `/cm toggle`.
...
- **Built-in default AFK/return message.** Players without their own message (and with
  ...
  built-in default so announcements still work for non-paid players once EssentialsX's
  own broadcasts are disabled. Mirroring EssentialsX's old behaviour, the built-in AFK
  default is silent while the return default is announced (`* <name> is no longer AFK.`).
  The admin default and `/cm toggle` opt-out still take precedence.
